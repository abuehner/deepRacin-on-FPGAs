...
gboolean dR_propagateShapesAndSchedule(dR_Graph* net){
	printf("dR_propagateShapesAndSchedule 0 \n");
    ...
    while(layer)
    {
        if(!layer->propagated)
        {
            ...
            {
            	printf("dR_propagateShapesAndSchedule 1 \n");
...
gboolean dR_generateAndCompileKernels(dR_Graph* net)
{
    ...
    gboolean ret;
    dR_Node* current_layer;
    printf("KernelGen 0,1 \n");
    dR_list_resetIt(net->scheduledLayers);
    current_layer = (dR_Node*)dR_list_next(net->scheduledLayers);
    printf("KernelGen 0,2 \n");
    while(current_layer){
        if(current_layer->generateKernel)
        {
            printf("KernelGen 0,3 \n");
            printf("current layer id: %i \n", current_layer->layerID);
            if(!current_layer->generateKernel(net,current_layer))
            {
                printf("KernelGen 0,4 \n");
        ...
        printf("KernelGen 0,5 \n");
        current_layer = (dR_Node*)dR_list_next(net->scheduledLayers);
        printf("KernelGen 0,6 \n");
    }
    ...
    ret = dR_clOfflineBuildWithSource(net, filenames, numberoffiles);
