...
gchar* g_strdup(const gchar *str){
	gchar* str2 = (gchar*)g_malloc(strlen(str)*sizeof(char));
	strcpy((char*)str2, (const char*)str);
	return str2;
}

int g_get_current_time(GTimeVal* result){
    time_t now;
    now = time(0);
    struct timeval tm;
    gettimeofday(&tm, NULL);
    result->tv_sec = (glong)tm.tv_sec;
    result->tv_usec = (glong)tm.tv_usec;
    return 1;
}
...
g_string_free(GString* str, gboolean){
	free(str);
}
