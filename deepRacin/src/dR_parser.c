...
dR_Node* dR_parseGraph(dR_Graph* net, gchar* path, dR_Node*** outnodes, gint* numnodes, dR_Node*** outfeednodes, gint* numfeednodes)
{
    ...
    char* graph_file = "graph.dr";//g_build_filename(path, "graph.dr", NULL);
    filepath = (char*)malloc((strlen(path)+strlen(graph_file))*sizeof(char));
    int end = 0;
    for(int i2 = 0; i2 <= strlen(path); i2++){
    	filepath[i2] = path[i2];
    	end = i2;
    }
    int c = 0;
    for(int i3 = end; i3 <= end+strlen(graph_file); i3++){
    	filepath[i3] = graph_file[c];
    	c++;
    }
    ...
    *numfeednodes = number_of_feednodes;
    ...
    for(i=0;i<number_of_nodes;i++)
    {
        ...
        for(it=0;it<num_variables;it++)
        {
        	...
        	printf("1it: %i p0: %s \n",it, params[0]);
            char* fullpath = (char*)malloc((strlen(path)+strlen(variableFilenames[it]))*sizeof(char));
            int end = 0;
            for(int i2 = 0; i2 <= strlen(path); i2++){
            	fullpath[i2] = path[i2];
            	end = i2;
            ...
            int c = 0;
            for(int i3 = end; i3 <= end+strlen(variableFilenames[it]); i3++){
            	fullpath[i3] = variableFilenames[it][c];
            	c++;
            }
          ...
          //  strcat(filepath, variableFilenames[it]);
            printf(fullpath);
             ...
             success = g_file_get_contents(fullpath,&filecontentVar,&length,&err);

             variables[it] = (gfloat*)filecontentVar;
        ...
        else if(strncmp(desc, "MaskDependentFilter", strlen("MaskDependentFilter"))==0)
        {
            ...
            printf("parse MaskDependentFilter \n");
...
dR_Node* dR_parseModel(dR_Graph* net, dR_Node* input, gchar* path, dR_Node** nodelist, gint* numnodes)
{
    ...
    filepath = "squeezenet_whole/graph.dr";//g_build_filename(path, "graph.dr", NULL);
    if(!net->config->silent)
        g_print("Loading Model File %s ...\n",path);
