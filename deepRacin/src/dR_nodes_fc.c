...
gboolean dR_fc_compute(dR_Graph* net, dR_Node* layer){
    ...
    printf("\nLayer %i: \n", layer->layerID);
    for(int i3 = 0; i3 < sizeof(fclayer->weights)/sizeof(fclayer->weights[0]); i3++){
        printf("Weight[%i]: %f \n", i3,fclayer->weights[i3]);
    }
//    if(layer->layerID == 1) printf("Weight[0]: %f \n",fclayer->weights[1]);
    printf("numInputNeurons: %i \n", numInputNeurons);
...
gboolean dR_fc_propagateShape(dR_Graph* net, dR_Node* layer)
{
    ...
    printf("fclayer->ishape.s0 %i \n", fclayer->ishape.s0);
    printf("fclayer->ishape.s1 %i \n", fclayer->ishape.s1);
    printf("fclayer->ishape.s2 %i \n", fclayer->ishape.s2);
...
/*//
    if(layer->layerID == 1){
    	fclayer->weights[0] = 0.5;
    	fclayer->weights[1] = -1.5;
    }else if(layer->layerID == 2){
    	fclayer->weights[0] = 1.5;
    }
//*/
...
{
    dR_FC_Data* fclayer = ((dR_FC_Data*)(layer->layer));
    if(fclayer->hasVariables)
    {
        g_free(fclayer->weights);
        if(fclayer->useBias)
        {
            g_free(fclayer->biases);
        }
    }
