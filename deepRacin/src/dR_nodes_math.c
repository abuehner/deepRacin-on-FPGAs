...
gboolean dR_softmax_compute(dR_Graph* net, dR_Node* layer){
    ...
    float* input;
    dR_list_resetIt(layer->previous_layers);
    input = (float *)((dR_Node*)dR_list_next(layer->previous_layers))->outputBuf->bufptr;
    ...
    int mult2;
    int gid;
	for(int gx = 0; gx < softmaxlayer->ishape.s0; gx++){
		for(int gy = 0; gy < softmaxlayer->ishape.s1; gy++){
			for(int gz = 0; gz < softmaxlayer->ishape.s2; gz++){
				mult = gz*(softmaxlayer->ishape.s0*softmaxlayer->ishape.s1);
				mult2 = gy * softmaxlayer->ishape.s0;
				gid = mult +(mult2+ gx);
				softmaxlayer->expsHost[gid] = exp(input[gid]);
			}
	...
	for(int gx = 0; gx < layer->oshape.s0; gx++){
		for(int gy = 0; gy < layer->oshape.s1; gy++){
			for(int gz = 0; gz < layer->oshape.s2; gz++){
				mult = gz*(layer->oshape.s0*layer->oshape.s1);
				mult2 = gy * layer->oshape.s0;
				gid = mult +(mult2+ gx);
				softmaxlayer->expsHost[gid] = input[gid]*sumExp;
			}

		}
	}

    return true;//dR_finishCLKernel(net, "deepRACIN:softmaxNorm");
    ...
    dR_downloadArray(net, "reduceExps", layer->outputBuf->bufptr, 0 , inputsize * sizeof(cl_float), softmaxlayer->expsHost);
...
///    dR_createKernel(net,"computeExp",&(softmax->clKernelComputeExp));
    return true; ///dR_createKernel(net,kernelname,&(layer->clKernel));
}
