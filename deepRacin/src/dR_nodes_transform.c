...
{/*
	printf("concat generateKernel 0\n");
	dR_Concat_Data* concatnode = ((dR_Concat_Data*)(layer->layer));
    gint length = 0;
    ...
    source = NULL;
	printf("concat generateKernel 1\n");
    //Creating Shader Name and Return type
	source = (char * ) malloc((strlen("#ifndef kerneldef_ \n"
		    "#define kerneldef_ \n\n"
		    "__kernel void (\n")+ 3*(sizeof(kernelname)))*sizeof(char));
	sprintf(source, "#ifndef kerneldef_%s \n"
		    "#define kerneldef_%s \n\n"
		    "__kernel void %s(\n"
   ...
   /* source = g_strdup_printf(
	...
	/*printf("concat generateKernel 2\n");
    for(i = 0; i<n; i++)
    {
    	printf("concat generateKernel 2,1\n");
    	char* format = "    const __global float *gInput%d,\n";
    	printf("Allocate %zu bytes \n", (strlen(format)+3)*sizeof(char));
    	temp = (char *)malloc((strlen(format)+3)*sizeof(char));
    	printf("g_strdup 1\n");
    	sprintf(temp, format, i);
    	/*temp = g_strdup_printf(
    "    const __global float *gInput%d,\n",&i);*/
  /*  	printf("concat generateKernel 2,2\n");
        source = concat_and_free_old(source,temp);
        free(temp);

    }
	printf("concat generateKernel 2,3\n");

	temp = (char*)malloc((strlen("    __global  float * gOutputImage\n"
		    ")\n"
		    "{\n"))*sizeof(char));
	sprintf(temp, "    __global  float * gOutputImage\n"
		    ")\n"
		    "{\n");
   /* temp = g_strdup_printf(
    ...
    ")\n"
    "{\n");*/
	/*printf("concat generateKernel 2,7\n");

    source = concat_and_free_old(source,temp);
	printf("concat generateKernel 3\n");
    for(i=0;i<n;i++)
    {
    	temp = (char*)malloc((strlen("    if(get_global_id()<);\n"
    	        "    {\n")+strlen(concatDim)+ strlen(concatnode->inputSizesInConcatDim[i]))*sizeof(char));
    	sprintf(temp, "    const __global float *gInput%d,\n", i);
    	/*temp = g_strdup_printf(
 ...
 /*       source = concat_and_free_old(source,temp);
        ...
        {
        	temp = (char*)malloc((strlen("       int gid = get_global_id(2)*+get_global_id(1)*+(get_global_id(0)+); \n")+strlen(oheight*owidth)+strlen(owidth)+strlen(acum))*sizeof(char));
        	sprintf(temp, "       int gid = get_global_id(2)*%d+get_global_id(1)*%d+(get_global_id(0)+%d); \n", oheight*owidth, owidth, acum);
 ...
 /*       }
        ...
        {
        	temp = (char*)malloc((strlen("       int gid = get_global_id(2)*+(get_global_id(1)+)*+get_global_id(0); \n")+strlen(oheight*owidth)+strlen(owidth)+strlen(acum))*sizeof(char));
        	sprintf(temp, "       int gid = get_global_id(2)*%d+(get_global_id(1)+%d)*%d+get_global_id(0); \n", oheight*owidth, owidth, acum);
 ...
 /*       }
        ...
        {
        	temp = (char*)malloc((strlen("       int gid = (get_global_id(2)+)*+get_global_id(1)*+get_global_id(0); \n")+strlen(oheight*owidth)+strlen(owidth)+strlen(acum))*sizeof(char));
        	sprintf(temp, "       int gid = (get_global_id(2)+%d)*%d+get_global_id(1)*%d+get_global_id(0); \n", oheight*owidth, owidth, acum);
  ...
  /*      }
   	printf("concat generateKernel 4\n");
        source = concat_and_free_old(source,temp);

    	temp = (char*)malloc((strlen("       int igid = get_global_id(2)*+get_global_id(1)*+get_global_id(0); \n"
    	        "       gOutputImage[gid]=gInput[igid];\n"
    	        "    }\n")+strlen(oheight*owidth)+strlen(owidth)+strlen(i))*sizeof(char));
    	sprintf(temp, "       int igid = get_global_id(2)*%d+get_global_id(1)*%d+get_global_id(0); \n"
  ...
  /*      source = concat_and_free_old(source,temp);
    ...
    "#endif \n");*/
 /*   source = concat_and_free_old(source,temp);
	printf("concat generateKernel 5\n");
	...
	printf("concat generateKernel 6\n");
    g_free(source);
*/
    return TRUE;
}
